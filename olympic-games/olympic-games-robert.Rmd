...
<<<<<<< HEAD
  view()
=======
>>>>>>> c41cc11eaf20d2a7be3a5075dde9ce4c833e7c2a
```
...
t5sports <- dff%>%
<<<<<<< HEAD
  select(game, sport, discipline, athlete, age, medal)%>%     # selecting these variables
  filter(!is.na(age))%>%                                      # removing those observations where age is NA
  filter(!is.na(medal))                                       # removing the non-medallists from the data frame
```
```{r}
t5sports %>%
  group_by(sport) %>% 
  summarise(mean(age), max(age), min(age)) %>% 
  arrange (desc(sport))
```

... this is the current state... I will initialize a pull request at this stage in order to get a feel for the review feature of git... and maybe a hint on this task. I am unsure how to split the analysis within the variable sports....
=======
